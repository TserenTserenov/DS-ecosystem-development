---
marp: true
theme: default
paginate: true
style: |
  section {
    font-family: Arial, sans-serif;
    font-size: 26px;
    padding: 40px 60px;
    background: #ffffff;
  }
  h1 {
    color: #1a3a5c;
    font-size: 38px;
    border-bottom: 3px solid #3d6cd0;
    padding-bottom: 10px;
  }
  h2 { color: #1a3a5c; font-size: 30px; }
  strong { color: #1a3a5c; }
  img { display: block; margin: 0 auto; }
  blockquote {
    border-left: 5px solid #3d6cd0;
    padding: 12px 20px;
    color: #334155;
    font-style: italic;
    background: rgba(61, 108, 208, 0.07);
    border-radius: 0 10px 10px 0;
  }
  table { font-size: 22px; }
  ul li, ol li { margin-bottom: 8px; }
  section.title {
    background: linear-gradient(135deg, #1a3a5c 0%, #0f2a44 100%);
    color: white; display: flex; flex-direction: column;
    justify-content: center; align-items: center; text-align: center;
  }
  section.title h1 { color: white; border-bottom: 3px solid rgba(255,255,255,0.4); font-size: 48px; }
  section.title p, section.title strong { color: rgba(255,255,255,0.85); }
  section.divider {
    background: linear-gradient(135deg, #3d6cd0 0%, #12308f 100%);
    color: white; display: flex; flex-direction: column;
    justify-content: center; align-items: center; text-align: center;
  }
  section.divider h1 { color: white; border-bottom: 3px solid rgba(255,255,255,0.4); font-size: 44px; }
  section.divider p, section.divider strong { color: rgba(255,255,255,0.92); font-size: 28px; }
  section.question {
    background: linear-gradient(135deg, #e67e22 0%, #c8560f 100%);
    color: white; display: flex; flex-direction: column;
    justify-content: center; align-items: center; text-align: center;
  }
  section.question h1 { color: white; border-bottom: 3px solid rgba(255,255,255,0.4); font-size: 38px; }
  section.question p, section.question li { color: rgba(255,255,255,0.95); }
  section.question ol, section.question ul { text-align: left; display: inline-block; }
  section.tip {
    background: linear-gradient(135deg, #1e8449 0%, #145a32 100%);
    color: white; display: flex; flex-direction: column;
    justify-content: center; align-items: center; text-align: center;
  }
  section.tip h1 { color: white; border-bottom: 3px solid rgba(255,255,255,0.4); font-size: 40px; }
  section.tip p, section.tip li { color: rgba(255,255,255,0.95); }
  section.tip ul { text-align: left; display: inline-block; }
---

<!-- _class: title -->

# Практикум S2 «Агентность»

**Программа «Личное развитие» Мастерской инженеров-менеджеров**

Церен Церенов · 14 сентября 2026

---

## Сегодня

- Что нового в программе «Личное развитие»
- Практикум S2 «Агентность»
- Как продолжить или выбрать рабочий проект
- Разбор ваших кейсов

---

<style scoped>
p, li { font-size: 19px; }
</style>

![bg fit right:70%](assets/bylo-stalo-project.svg)

## Продолжение или новый рабочий проект

**Если прошли S1** — открываем прежнюю карточку: продолжить тот же проект на рабочей задаче, переформулировать или заменить.

**Если пришли прямо на S2** — выбираем безопасный рабочий проект с нуля, тем же путём.

Сохраняются метод и история свидетельств — не тема проекта.

---

![h:640](assets/mandatory-to-project-focus.png)

---

![h:600](assets/session-three-results-clean.png)

---

<!-- _class: divider -->

# 10 минут

**Пишите в свою заметку, в чат — только рабочий продукт недели**

Вслух разберём 2-3 примера

---

## Матрица 4 мастерства: где стоит S2

|  | Управлять | Развивать |
|---|-----------|-----------|
| **Человек (ты сам)** | внимание, время, состояние, ритм | методы ученика, ступени |
| **Инструмент (IWE)** | оператор, не пассажир | достраивать под себя |

**S2 «Агентность» развивает верхнюю строку** — управление и развитие себя. Практикум по IWE — нижнюю.

> Инструмент сменный. Мастерство и мировоззрение остаются с вами навсегда.

---

## 8 практик саморазвития

<style scoped>
table { font-size: 19px; }
</style>

| # | Практика | Управляет | Развивает |
|---|----------|-----------|-----------|
| 1 | Инвестирование и учёт времени | распределение времени | ритм |
| 2 | Медленное чтение | внимание при чтении | глубину мышления |
| 3 | Мышление письмом | фокус мысли | ясность |
| 4 | Мышление проговариванием | речевое внимание | объяснение |
| 5 | Организация досуга | восстановление | устойчивость |
| 6 | Формирование окружения | среду вокруг себя | поддержку роста |
| 7 | Стратегирование | направление | траекторию |
| 8 | Планирование | приоритеты | исполнительскую культуру |

---

## Основной показатель — систематичность

![w:950](assets/ladder-5-stages.svg)

---

<!-- _class: question -->

## Где на этой лестнице начинается S2

Практикум «Агентность» закрепляет ритм ступеней 1-2, ведёт к ступени 3, закладывает фундамент ступени 4.

**В чат:** какая ступень сейчас кажется вам ближайшей?

---

![w:1100](assets/ladder-stages-overview.png)

---

## Другие отслеживаемые показатели

Систематичность — главный, но не единственный показатель.

- **Агентность** — в фокусе этого практикума
- Мировоззренческая зрелость
- Системность
- Оснащённость
- Собранность (Систематичность, Ритмичность, Внимательность)

> [нужна картинка] версия `learner-metrics.png` с выделенным блоком «Агентность» вместо «Собранность».

---

![h:620](assets/creator-foundation-pyramid.png)

---

## Что растим на каждом практикуме

![w:1000](assets/cascade-focus.svg)

---

## Развитие Мастерства

![h:600](assets/pilot-mashina-sverhchelovek.png)

---

## Персональное руководство пересобирается из вашей активности

![w:520](assets/knowledge-extraction.png)

Три источника: **ступень и узкое место** (диагностика) · **ваша реальная жизнь и работа** · **ваш запрос**.

---

![h:540](assets/ai-environment-before-after.png)

**S1** — браузерная версия IWE + личный проект, **S2** — терминальная версия IWE (VS Code или аналоги) + рабочий проект

---

## Продолжить, переформулировать или заменить проект

<style scoped>
p, li { font-size: 22px; }
</style>

**Если проходили S1:** откройте прежнюю карточку, прогоните четыре пункта заново — сохраните проект или замените.

**Если пришли сразу на S2:** пройдите четыре пункта с нуля.

**Стоп-кран:** проект обезличен · без секретов и конфиденциальных данных · не медицинское/юридическое/инвестиционное решение · не поручает ИИ внешние действия · результат под вашим контролем.

---

<!-- _class: question -->

## Живое упражнение: инструкция

1. Что сейчас не устраивает или хочется сдвинуть
2. Зачем это менять
3. Рабочий продукт недели — конкретная вещь, которую покажете через неделю
4. Как поймёте, что получилось

---

## Разбор через формулу «один сеанс — три результата»

Берём 2-3 прозвучавших рабочих продукта и разбираем вслух:

- откуда взялась неудовлетворённость
- какой отсюда рабочий продукт недели
- где здесь движение проекта, где рост мастерства, где усиление среды

---

## Как устроен практикум

<style scoped>
p, li { font-size: 21px; }
ol li { margin-bottom: 4px; }
</style>

![w:1230](assets/weekly-rhythm.svg)

Что делать на первой неделе:

1. Продолжить, переформулировать или заменить рабочий проект
2. Настроить терминальную версию IWE (смотрите в следующем слайде)
3. Начать учёт времени — цель 3-5 часов за 7 дней
4. Изучить раздел 1 руководства, сделать заметки
5. Определить рабочие продукты недели и стараться делать их каждый день

---

## Настройка терминальной версии IWE

> TODO: точные ссылки на подключение (VS Code / аналоги) — уточнить у пилота перед встречей, готового поста-аналога браузерных инструкций S1 не найдено.

Обсудите с ИИ:

- зачем именно вам терминальная версия (в отличие от браузерной)
- чем терминальная версия отличается от браузерной
- как начать разворачивать её на своей машине

---

<!-- _class: question -->

# Вопросы

---

## Рефлексия и закрытие

![w:700](assets/path-to-team.png)

**Один инсайт — в чат, одной фразой.**
