---
created: 2026-07-08
type: presentation-outline
event: Кикофф второй волны когорты — командная работа + онбординг + руководство
format: живая встреча + Telegram-голосование
source: adapted from Marathon-Wave1-2026/kickoff-presentation.md
related_wp: [WP-437, WP-330]
---

# Кикофф волны-2 — план встречи

> Наследует структуру и тон кикоффа волны-1. Отличия отмечены **[волна-2]**.

---

## Слайд 1 — Открытие

**Добро пожаловать во вторую волну!**

Сегодня:
- Познакомимся с командой
- Разберём, зачем всё это
- Расскажем, как работаем — **в этот раз в двух форматах**
- Поговорим о главном — внимании
- Ответим на вопросы

*Ведущий: [имя]*

---

## Слайд 2 — Команда

**Кто рядом на волну**

*(Каждый — 15-20 секунд: имя, откуда, что делает в жизни)*

- Подгаевский Александр
- Заводчикова Марфа
- Чайковская Юлия
- Чайковская Алена
- Церен Церенов

*Мы здесь не лекторы — мы те, кто прошёл похожий путь.*

**[волна-2]** В этот раз без персональных наставников — команда держит себя сама через протокол (см. слайд 5).

---

## Слайд 3 — Цель волны

**Систематичность, а не сразу качество**

Ошибка новичка: ждать, пока будет «правильно» и «достаточно глубоко».

Цель — та же, что в волне-1:
> Удерживать внимание на своём развитии каждый день.

Время, глубина, результат — вторично.
Регулярность — главное.

**[волна-2]** Мы также проверяем: **работает ли команда лучше, чем в одиночку**. Это часть эксперимента, честно говорим прямо сейчас.

---

## Слайд 4 — Голосование: как ты обычно?

**Выбери варианты, которые ближе всего:**

1. 🔄 Стартую с энтузиазмом — и бросаю через неделю-две
2. 🧱 Застреваю: начал, что-то непонятно, и дальше не двигаюсь
3. 🌀 Хаос: не понимаю с чего начать, всё кажется важным
4. 🔒 Держусь за убеждения: «это не для меня», «у меня по-другому»
5. ⏰ Делаю в последний момент: накануне дедлайна, на авось
6. 📱 Нет времени: всегда что-то срочное вытесняет саморазвитие

*(Голосование в боте / чате — результаты разбираем вместе)*

---

## Слайд 5 — Как работаем **[волна-2: два трека]**

**Ты идёшь одним из двух путей — выбор сегодня**

**Командный под (6-8 человек)**
- Еженедельный протокол: 4 вопроса, 45-60 минут
- Есть «якорь» — таймкипер из выпускников волны-1, держит ритм встречи (не наставник, не проверяет вас — держит время и порядок вопросов)
- Общий результат — одно совместное различение к концу волны

**Соло-трек (контрольная группа)**
- Тот же путь: онбординг + бот + рабочая тетрадь + персональное руководство
- Без командных встреч — как в волне-1

*Оба трека получают персональное руководство и доступ к боту @aist_me_bot.*

---

## Слайд 6 — Честно про эксперимент **[волна-2, новый слайд]**

**Это пилотная волна — говорим прямо**

- Волна платная (цена основателя — ниже, чем будет дальше)
- Если у команды не сложится ритм (меньше половины встреч или заметный отток) — переключаемся на соло-формат
- Без отката назад и без возврата денег — рычаг в таких случаях один: продлеваем срок обучения, а не сокращаем его
- Ваша обратная связь — часть результата волны, не формальность

---

<!-- изображение: цель / goal-box -->

---

## Слайд 7 — О состояниях системы

**Ты — система. У системы есть состояния.**

Прямо сейчас у тебя есть привычный стиль жизни — набор автоматических состояний:
- Как ты реагируешь на усталость
- Что делаешь в свободные 15 минут
- Когда «в ресурсе», а когда нет

Волна не учит тебя «работать над собой».
Волна меняет одно состояние: **как ты распоряжаешься своим вниманием**.

---

## Слайд 8 — Откуда берётся внимание **[волна-2, новый слайд]**

**У каждого уже сложились привычки потребления информации**

Мы получаем информацию из десятков каналов - лента, мессенджеры, короткие ролики. Совсем не потреблять информацию мы не можем, так устроен мозг. Но мы неосознанно тянемся к простому и привычному - там не нужно прикладывать усилие.

Чтобы что-то изменить, нужно отвоевать внимание у этих каналов. И начать не с самого сложного, а с простого действия - чтобы почувствовать сам слом привычного поведения.

> Стакан воды перед едой - осознанно, не на автомате. Три вдоха перед тем, как взять телефон. Один приём пищи без экрана.

Та же мышца, что понадобится для ежедневного слота.

---

## Слайд 9 — Стать хозяином своего внимания

**Задача простая — и сложная одновременно**

> Удержать внимание на своём развитии как можно больше дней подряд.

Не «освоить всё». Не «стать другим человеком».
Просто: сегодня я открыл материал. Завтра — тоже.

Когда это становится привычкой — меняется не только знание.
Меняется то, как ты принимаешь решения, куда смотришь, что замечаешь.

*Это и есть стиль жизни — не режим, а естественное состояние.*

---

## Слайд 10 — Правила участника

**Общие для обоих треков:**

1. **Удерживай ежедневный слот.** Время и сложность — вторично, главное — регулярность.
   Читай в боте или тетради.

2. **Пиши заметки.** Если хочешь — добавь рефлексию: что удивило, что зацепило.

3. **Будь активен:** начни с вопросов в чате, делись текстами.

**Если ты в командном поде — добавляется четвёртое:**

4. **Приходи на еженедельный синк.** 45-60 минут, 4 вопроса протокола. Пропуски считаются — это часть того, что мы измеряем.

---

## Слайд 11 — Вопросы и ответы

**Что непонятно? Что беспокоит?**

*(5-10 минут живого диалога)*

---

*Удачи. Мы рядом.*
