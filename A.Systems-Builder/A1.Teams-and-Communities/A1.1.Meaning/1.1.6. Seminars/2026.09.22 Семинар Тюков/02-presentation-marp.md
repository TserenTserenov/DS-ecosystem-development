...
footer: "v0.4"
role: DP.ROLE.060
note: "Дека по 01-scenario.md (SoT содержания - править сценарий и пересобирать). v0.4 (22.09, сокращение пилотом перед выступлением): убрано 13 слайдов для тайминга - «Три типа ИИ-помощников»; «Одна система - три уровня описания» и «Ваш пилот - такая же система» (общая таблица открывателя, живой кейс-пример остался); схема и слайд «Скрипт ≠ Скилл», антипример «Что бывает без роли-проверяющего», «Роль-автомат Экстрактор» (сам конвейер и картинка остались); практика «Извлеките знание из своего разговора»; «Шаблон описания пилота», «Пример: инструкции для новых сотрудников», «Расширенный шаблон - одна повторяющаяся работа за раз» (сама практика «Разложите свой пилот» осталась); «Четыре рычага»; «Куда дальше - три двери» (сами слайды подключения остались). 30 слайдов итого. v0.3 (22.09, Archive.zip заказчика разобран): в блок 4 добавлены пример-разбор реального кейса участника акселератора (агент для постов, роль-проверяющий) и антипример без роли-проверяющего (финансовый агент); в блоке 5 общий пример заменён на реальный кейс участника (инструкции для новых сотрудников, 9ч → 5ч, полнота 90%) - антипример и шаблонный пример позже сняты в v0.4 при сокращении. v0.2 (22.09, решение пилота): добавлены «Роль-автомат Экстрактор», схема knowledge-extraction.png, «Конвейер извлечения знания»; в закрытии - «14 элементов культуры» и «Четыре рычага» с тупиком «печатная машинка» (перед «Что унести»). Тема may31 (theme-seminar.css скопирован из 06.09 без изменений). Сборка: marp --allow-local-files 02-presentation-marp.md --theme-set=theme-seminar.css --pdf -o 02-presentation-marp.pdf"
style: |
