---
status: delegated-to-wp-281
updated: 2026-06-09
---

# WP-183: Единый учёт оплат и автоматизация допуска

> **Статус:** Фаза A частично выполнена. Dual-write активен. Шаги 7/8/9 → WP-281 Э3 (W27–W28, июль 2026). Фаза B (Ory) частично. Фаза D (Stripe/Track B) — stub, ждёт GKE.
> **Участники:** Дима (Aisystant), Ильшат (владелец Track A с W23), Гиляна (бухгалтерия), Алёна (маркетинг), Юля (декан), Церен и Андрей (архитектура)

---

## 🔄 Актуализация 29 мая 2026

**Источник:** WP-281 (мастер-план Track A, 28 мая 2026), дедлайн 15 июля.

**Владелец изменился:** Ильшат принимает Track A с W23 (3 июня). Церен передаёт доступы, код и контекст.

**Текущий статус шагов Фазы A** (обновлено по WP-281):

| Шаг | Задача | Статус |
|-----|--------|--------|
| 1 | Схема `finance.payments` в Neon | ✅ Выполнен (апр) |
| 2 | Экспорт истории (19684 payment + 16155 chargeoff) | ❓ Не подтверждён — уточнить у Димы |
| 3 | Dual-write: Aisystant → Neon | ✅ Активен |
| 4 | Aisystant читает из Neon | ✅ Активен |
| 5 | Directus на Railway | ✅ Done |
| 6 | Metabase на Railway | ✅ Done |
| 7 | Сверка 24ч | 📅 Плановый → WP-281 Э3 (3.5, W27–W28) |
| 8 | Автодопуск + маршрутка (8 каналов) | 📅 Плановый → WP-281 Э3 (3.3, W27–W28) |
| 9 | Индивидуальные ссылки YooKassa | 📅 Плановый → WP-281 Э3 + Дима |

**Фаза B (Ory):** Ory в prod работает (апр). Единый вход в Directus (замена basic auth) → WP-281 Э3 задача 3.4 (Андрей, W27–W28).

**Фаза D (Track B, Stripe) — stub:** Мировая платформа использует Stripe (решение встречи 14). Отдельный платёжный канал для GKE (Track B). Детальное проектирование — после запуска GKE кластера (Паша, WP-285).

---

## 🔄 Актуализация 7 мая 2026

**1. Архитектура финансовой БД — финализирована (DP.ARCH.004 v2.4, апр)**
БД называется `finance` (не `payment-registry` как обсуждалось 19 апр). Это одна из 12 entity БД в Neon. Connection string обновлён в DP.ARCH.004.

**2. Cut-over прошёл (26–30 апр, WP-268)**
Переход на новую архитектуру состоялся. Aisystant (LMS) теперь работает в mutual read-only режиме с Neon. Dual-write активен (шаг 3 Фазы A). Бот читает оплаты из Neon.

**3. Статус шагов Фазы A**

| Шаг | Задача | Статус |
|-----|--------|--------|
| 1 | Схема `finance.payments` в Neon | ✅ Выполнен 10 апр |
| 2 | Экспорт истории (19684 payment + 16155 chargeoff) | ❓ Статус неизвестен — уточнить у Димы |
| 3 | Dual-write: Aisystant → Neon | ✅ В работе (cut-over 26–30 апр) |
| 4 | Aisystant читает из Neon (баны, подписки) | ✅ Активно (часть новой архитектуры) |
| 5 | Directus на Railway | ✅ 8 апр |
| 6 | Metabase на Railway | ✅ 8 апр |
| 7 | Сверка 24ч | ❓ Уточнить у Димы |
| 8 | Автодопуск + маршрутка | ❓ Уточнить |
| 9 | Индивидуальные ссылки YooKassa | ❓ Уточнить |

**4. Фаза B (Ory) — частично выполнена**
Ory запущен в prod (апрель). Основные аккаунты мигрированы. Детали Фазы B (dual-write отключение, единый вход Directus) — уточнить текущий статус.

**5. Новое: Track B (мировая платформа) — Stripe**
Решение встречи 14 (7 мая): мировая платформа (Track B, GKE) использует **Stripe**, не YooKassa. Это отдельный платёжный канал, не описанный в этом документе. Канал 9 (Stripe) нужно добавить в схему при проектировании Track B оплат.

**6. autopay-колонки — классифицированы как `payment_credentials`**
DP.ARCH.004 §2 П6.1 подтверждён: `autopay`, `autopay_data`, `auto_extend` — класс `payment_credentials` (строже PII), Fernet-шифрование + RLS. Реализация: WP-246.

**Следующие шаги по этому документу:**
- Уточнить у Димы статус шагов 2, 7, 8, 9
- Добавить раздел «Фаза D: Track B (Stripe)» для мировой платформы
- После уточнения статуса — обновить таблицу шагов выше

---

## 🔄 Актуализация 19 апр (встреча с Андреем, оперативка ИТ)

> Три архитектурных сдвига, влияющих на этот документ. Сам дизайн (dual-write, 1 таблица, 8 каналов) **не меняется** — меняется размещение и класс чувствительности.

**1. Payment Registry → отдельная БД #4 `payment-registry`** (WP-228 ArchGate, 19 апр).
- Ранее: `finance.payments` в БД `directus` (применено 10 апр как временное решение).
- Теперь: отдельная БД `payment-registry` в Neon, схемы `finance.*`, `crm.*`. Обоснование — bounded context + blast-radius для платёжных данных.
- **Что делать:** миграция схемы `finance.*` из БД `directus` → БД `payment-registry` (плановая, в рамках WP-228 Ф9). Dual-write переключается на новый connection string.
- **Связь с документом:** §4 схема остаётся; §7.3 диаграмма требует правки (Neon PG → payment-registry DB #4).

**2. Autopay — портирование из LMS, не новый дизайн** (WP-246 Стадия 3, 17 апр).
- Класс чувствительности `payment_credentials` введён в [DP.ARCH.004 §2 П6.1](../../../PACK-digital-platform/pack/digital-platform/02-domain-entities/DP.ARCH.004-neon-data-architecture.md) — **строже PII**: логирование запрещено даже в маскированном виде, только зашифрованное хранение, доступ по service account с аудитом.
- YooKassa `payment_method_id` + токены автосписаний переносятся из LMS **без переизобретения** (решение LegacyPortGate 17 апр).
- CF Worker `payment-scheduler` (cron-списания) — реализуется в рамках WP-246 Ф10-Ф16 (май-июнь, ~49h).
- **Связь с документом:** колонки `autopay`, `autopay_data`, `auto_extend` в §4.1 — корректны; нужно явно пометить их как `payment_credentials` для RLS/шифрования.

**3. Q2-режим: «не развиваем, поддерживаем под MVP».**
- Не замораживается (продолжаем): Фаза A dual-write, сверка 24ч, миграция истории.
- Откладывается до после MVP (после 1 июня): Фаза B (Ory-миграция аккаунтов), Фаза C (Points, Revenue Sharing, юнит-экономика).
- **Связь с документом:** §3 Фазы B и C остаются в плане, но срок — «после теста гипотезы массового продукта».

**Для встречи 10 (вторник 21 апр):** согласовать (а) перенос `finance.*` в БД #4 (timing), (б) классификацию колонок autopay как `payment_credentials` (кто реализует RLS — Дима или Tseren через wrapper), (в) приоритет миграции истории 19684 payment + 16155 chargeoff (в Q2 или отложить до после MVP).
...
**2.2.** ~~Дима: схема `finance.payments` в Neon~~ → ✅ **Согласовано 8 апр.**
- Все колонки на месте — подтверждено
- **Ключевое решение:** при dual-write создавать/обновлять **1 строку** (не 2 отдельных INSERT для payment и charge_off). При миграции истории — та же логика (1 строка). Fallback: если миграцию в 1 строку не получится — передавать отдельно chargeoff и payment, как описано в §4
- Маппинги `purpose`, `status`, `channel` — без замечаний
...
|---|-----|-----|-----|------|-----------|
| 1 | ~~**Схема `finance.payments` в Neon**~~ — 1 таблица (§4) | Церен | -- | ✅ 10 апр | SQL закоммичен 8 апр, применён к Neon 10 апр (database `directus`, schema `finance`, 31 колонка, 10 индексов). Расхождение 8→10 — инцидент «DONE без верификации». [SQL](WP-183-001-create-finance-payments.sql) |
| 2 | **Экспорт истории** из Aisystant PG → Neon | Церен + Дима | 1 + доступ Паша (✅ есть) + связка chargeoff→payment (Дима) | W15-16 | Планируется: 19684 payment + 16155 chargeoff → `finance.payments`. **NB:** реальное имя таблицы в Aisystant — `chargeoff` без подчёркивания. `purpose` — integer, требует mapping в TEXT enum. 8 апр по ошибке помечено ✅ — фактически не выполнено, верифицировано 10 апр |
...
| 5 | ~~**Directus на Railway**~~ + ручной ввод (Монета, B2B) | Церен | 1 | ✅ 8 апр | Развёрнут: [directus-production-d62d.up.railway.app](https://directus-production-d62d.up.railway.app). БД `directus` в Neon. Login: admin@system-school.ru |
| 6 | ~~**Metabase на Railway**~~ + дашборды | Церен | 5 | ✅ 8 апр | [metabase-production-a4f6.up.railway.app](https://metabase-production-a4f6.up.railway.app). Setup wizard через UI |
...
> **Уточнение Димы (8 апр):** payment и charge_off создаются/обновляются **в одной транзакции**. Связку charge_off → payment для миграции истории Дима сделает сам.

**Dual-write: 1 INSERT в одной транзакции**

Payment + charge_off сохраняются вместе → один INSERT в Neon с `charged_off_at` сразу:
```
-- Внутри транзакции, после save(payment) + save(chargeOff):
INSERT INTO finance.payments (
    ...
    NOW(), chargeOff.details, chargeOff.potokId
);
```

> Если payment создаётся без charge_off (неуспешная попытка) — тот же INSERT, но `charged_off_at = NULL`, `success = false`.

**Миграция истории:** Дима делает связку charge_off → payment.id. После этого — экспорт в 1 строку по связке.

**Как читать:**
- Все платежи: `SELECT * FROM finance.payments`
- Только успешные списания: `SELECT * FROM finance.payments WHERE charged_off_at IS NOT NULL`
...
CREATE UNIQUE INDEX idx_payments_upsert ON finance.payments (source_id, source_system) WHERE source_id IS NOT NULL;  -- для UPSERT (вариант A)
```
