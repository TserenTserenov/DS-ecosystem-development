...
**Удалено пилотом 03.09** (решение принято, репозитории с GitHub убраны — подтверждено `gh repo view`): `DS-my-strategy-dashboard` (стрей-дубль `DS-my-strategy`), `desktop-tutorial` (обучающий репозиторий GitHub Desktop, к IWE не относится), `srt-template1` (отдельный проект «Planora» на методе SRT, не структура IWE). Локальные клоны `~/IWE/DS-my-strategy-dashboard`(2-я копия, появилась заново — см. ниже) и `~/IWE/srt-template1` осиротели (origin удалён) — безопасны к удалению, содержимого без пары на GitHub не осталось.
...
**Ждут решения пилота (не тронуты):** `DS-test-wp514-20260819-151751`, `DS-test-wp514-e2e`, `DS-test-wp514-final-verify` — все три собственным описанием просят удаления после проверки (WP-514, 19-30.08).
...
**Находка 03.09 (повтор паттерна WP-520/WP-526):** прямо во время этой сверки в `~/IWE/DS-my-strategy-dashboard` появился НОВЫЙ полноценный `git clone` канона `DS-my-strategy` под тем же неудачным именем (не воркт-ри, отдельный `.git`) — судя по коммитам внутри, его сделала одна из параллельных сессий (WP-417/WP-514/WP-484), не эта. Дерево идентично origin (0 расхождений), удалять безопасно, но сам факт повторного появления клона под этим именем — живой симптом: что-то в тулинге параллельных сессий продолжает клонировать `DS-my-strategy` в directory `DS-my-strategy-dashboard` вместо использования изолированного worktree. Стоит найти источник (какой скрипт/адаптер выводит это имя), иначе будет повторяться.
