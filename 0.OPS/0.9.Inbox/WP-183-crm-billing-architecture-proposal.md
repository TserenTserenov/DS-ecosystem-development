...
**Два источника данных. Directus — окно в оба. Инструменты — тонкие слои.**

Внешние CRM (Twenty, EspoCRM, NocoDB) создают вторую БД и становятся центром (SPOF). Наше решение — CRM это **окно в данные**, не центр. Центр — бот + Aisystant PG + Neon + Ory (как в DP.ARCH.001).

**Два datasource:**
- **Aisystant PG** = source-of-truth для подписок, курсов, оплат через Tilda (каналы 1–5)
- **Neon PG** = source-of-truth для семинарных оплат через бота (каналы 6–7), CRM-данные (crm.\*, finance.\*)
- **Directus** подключается к обеим БД: read-only к Aisystant PG, read-write к Neon

> Полная картина двух datasource и унифицированного представления: [WP-183-unified-payments-proposal.md](WP-183-unified-payments-proposal.md)

- **Directus** = UI для руководителя (формы, группы, RBAC). Видит обе БД. Снимается без потери данных
...
- **Neon PG** = источник правды для бот-платежей и CRM (crm.\*, finance.\*)
- **Aisystant PG** = источник правды для подписок и курсов (существующие данные)
...
| **Billing Module** | Strategy pattern: 7 адаптеров. Модуль бота с чёткой границей | Своё | В составе бота |
...
| **Монета/PayAnyWay** | Оплата через Tilda (каналы 1–5, Aisystant PG) | email |
| **Aisystant Corp (USA)** | Оплата для международных клиентов | email |
...
Billing Service (orchestrator)
  ...
  ├── Монета/PayAnyWay (РФ, Tilda — каналы 1-5, Aisystant PG)
  ├── Aisystant Corp   (USA, международные клиенты)
  ├── Stripe           (мир, подписки)
