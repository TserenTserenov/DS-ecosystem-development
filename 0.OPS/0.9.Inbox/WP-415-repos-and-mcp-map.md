---
type: reference
title: "Карта репозиториев: доступ через MCP и локально"
status: active
...
> Источник истины для вопроса «какие репо подключить и как».
> При разделении организаций (Россия/Мир, WP-415) — обновить этот файл.
...
> **Командный MCP (future):** когда появятся облачные агенты команды (headless, без локальных файлов), им понадобится отдельный MCP для чтения репо из Секции 2 ниже. Для разработчиков в VS Code сейчас достаточно клона + Aisystant MCP.
