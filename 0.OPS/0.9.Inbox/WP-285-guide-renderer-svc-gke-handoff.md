...
> **Актуализация 2026-09-10 (пир-сессия Claude+Codex, WP-502):** handoff Андрею состоялся 25.06 и формально принят, но не архивируется — GKE-контур по факту ещё не развёрнут, CronJob остаётся в статусе pending (`inbox/WP-285/WP-285.md`), и после решения временно работать на `tsekh-1` список «20 сервисов», распределение владельцев и роль `guide-renderer-svc` ниже нуждаются в пересверке перед тем, как этот план станет исполняемым.
