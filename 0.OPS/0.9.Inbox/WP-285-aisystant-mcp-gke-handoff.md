...
> **Состояние перехода (на 2026-06-14).** Чистый шлюз без общих паролей — это **целевое** состояние, к которому ведёт WP-410. В текущем коде шлюза ещё живы **два** внутренних пароля, оба закрываются в WP-410:
> - **USER_PROFILE** — снятие подготовлено и отревьюено. Шаг A (вливание инертной чистки кода) делает текущий код соответствующим этому документу. **Важно для Track B:** пока шаг A не влит, свежая выкатка по списку переменных раздела 2.2 даст `503` на `/health` — текущий код требует `USER_PROFILE_SHARED_SECRET` в health-проверке. Решение: влить шаг A до деплоя Track B, либо временно задать этот пароль.
> - **SCOPE (bridge-scope)** — уходит после выдержки теневой проверки (≥7 дней, ориентир ~20 июня), затем проверка прав переезжает внутрь `personal-knowledge-mcp`.
...
- ⏳ Авторизация без общих паролей — **цель, не достигнута на 2026-06-14.** Сервисы проверяют личность сами (раздел 2.1), но в коде шлюза ещё живы два внутренних пароля (USER_PROFILE, SCOPE) — оба уходят через WP-410 к ~20 июня (см. «Состояние перехода» в шапке). Для свежего Track B-деплоя это значит: либо влить чистку WP-410 до деплоя, либо временно задать эти пароли.
