...
Платформа Aisystant строится вокруг одной идеи: специалист должен расти в системном мышлении не через абстрактные курсы, а через реальную работу — с ИИ-помощником рядом. IWE (Intellectual Work Environment) помнит паттерны ошибок, предлагает методы в нужный момент, автоматизирует рутину. Параллельно платформа ведёт человека по ступеням мастерства Ученика — от «случайного» применения методов до осознанного исполнения ролей и собственного вклада в развитие инструментов. Это не LMS с курсами, это среда профессионального роста.

Архитектурно платформа реализует **Букварь-модель (Из Алмазного века)**: адаптация происходит к росту конкретного пользователя, а не к масштабу по числу пользователей. Для каждой ступени Ученика меняются три вещи — интерфейс (упрощается или расширяется), содержание (от рецептов к философии методов) и доступные сервисы (от базовых до пользовательских микроприложений). Безопасность данных гарантирована архитектурно через **Parliament Model**: ни один агент не имеет доступа ко всем данным сразу — только к своей зоне ответственности через явный контракт (Service Clause), с независимым верификатором.

---

<details open>
<summary><b>1. Data Layers: Трёхслойное разделение данных</b></summary>

Все остальные части платформы (events, Parliament Model, Hermes) строятся поверх этих трёх слоёв.
...
<details>
<summary><b>2. Event Sourcing: От действия к знанию</b></summary>
...
<details>
<summary><b>3. Parliament Model: Триада Учёт / Доступ / Аудит</b></summary>
...
<details>
<summary><b>4. Service Architecture: Domain-Isolated Services</b></summary>
...
<details>
<summary><b>5. Hermes Agent: Память и самокоррекция</b></summary>

Hermes — агент верхнего уровня, который использует Data Layers (читает indicators, fault_patterns), работает через Parliament Model (только свой scope), вызывает сервисы. Понятен только после предыдущих разделов.
...
<details>
<summary><b>6. Session Flow: Всё вместе</b></summary>
...
Context (Слой 3, ephemeral prompt)
  ↓ Hermes + Services (через Parliament Model)
...
> **Текущий инвентарь сервисов (Track A → Track B):** [WP-285-services-inventory.md](WP-285-services-inventory.md) — 16 CF Workers + Python, 16 БД Neon, план миграции на GKE + Cloud SQL. Этот раздел описывает целевую архитектуру post-MVP, не текущее состояние.

### Фаза 1 MVP — Single-region, Neon
```
...
Neon (EU region):
  ...
  ├─ journal DB     (event sourcing)
  └─ audit DB       (access_log, Parliament Model)
    ↓
SQLite (local per user workspace):
  └─ iwe_memory.db  (Hermes: fault_patterns, checklists)
```

### Фаза 2 — Per-domain scaling (Q3-Q4 2026)
```
- learning domain → отдельный Postgres (read-heavy)
- audit domain → ClickHouse (time-series analytics)
- community domain → отдельная БД

Parliament Model делает это легко:
каждый domain agent говорит только со своей БД.
```

### Фаза 3 — Production (Q1 2027+, GKE)
```
Аналогично Track B (WP-285):
- GKE Standard (europe-west4)
- Cloud SQL вместо Neon
- Ory EU (Kratos + Hydra)
- Artifact Registry + Werf деплой
```

### Cost estimate (Фаза 1)
...
5. **Coordinator state:** stateless координатор — нужно ли кэшировать разрешения? (Redis / один вызов)
6. **WP-285 gap:** `subscription.contract_event` пустая 6 недель при 541 active subscriptions — до migration в Track B нужна диагностика projection-worker (WP-228 Ф32)
