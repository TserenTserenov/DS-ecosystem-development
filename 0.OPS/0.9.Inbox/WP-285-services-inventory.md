...
## 1. CF Workers (Cloudflare) — 11 сервисов
...
**Что нужно сделать одно общее:** создать новый `wrangler.toml` или `.env` под Track B для каждого воркера. Деплоить в тот же Cloudflare-аккаунт, но с другими именами и привязками.

---

## 2. Python-сервисы (контейнеры) — 5 сервисов
...
## 3. Базы данных — 12 Neon → 12 Cloud SQL
