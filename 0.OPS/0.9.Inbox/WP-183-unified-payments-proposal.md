...
> **Участники ревью:** Дима (Aisystant), Гиляна (бухгалтерия), Алёна (маркетинг), Юля (декан), Церен (архитектура).
...
**Общий бюджет:** ~13h (без учёта согласований). Это scope «единый учёт + допуск». Полный Billing Service (адаптеры, баллы, revenue sharing) — отдельный scope из WP-183 (~88h).
...
13. **Paybox** — планируется ли отключение? Или оставляем как резерв?
14. **PayPal** — упоминается в WP-73, но не реализован. Нужен?
...
| **WP-115** | Сценарий семинара — синхронизировать с реализацией |
