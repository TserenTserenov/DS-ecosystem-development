...
**Биллинг-логика пока остаётся в Aisystant** — не переносим сейчас, заберём позже. `charge_off` (успешные списания) = основа для решений, не `payment` (все попытки).
...
**2.2.** Дима: схема 3 таблиц в Neon (`finance.payments`, `finance.charge_offs`, `finance.chat_joins`) → [§4](#4-схема-finance-в-neon)
- Все ли поля на месте для dual-write из Aisystant?
