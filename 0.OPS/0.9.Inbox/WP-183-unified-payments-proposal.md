...
**Биллинг-логика остаётся в Aisystant** — не переносим. `charge_off` (успешные списания) = основа для решений, не `payment` (все попытки).
...
**2.2.** Дима: схема 3 таблиц в Neon (`finance.payments`, `finance.charge_offs`, `finance.chat_joins`) → [§4](#4-схема-financе-в-neon)
- Совместима ли с биллинг-логикой Aisystant?
- Все ли нужные поля на месте?
...
|---|-----|-----|-----|------|-----------|
| 1 | **Схема `finance.*` в Neon** (§4) | Церен | -- | W15 | Готовая схема. Дима валидирует |
...
**Принципы:**
- Совместимость с биллинг-логикой Aisystant (М3)
- `double → decimal` (округление до 1 руб.)
- `purpose` как string enum (не ordinal int)
- Новые поля: `channel` (1-8), `source_system`, `notified_bot`
- Мягкое удаление: `archived_at` (Д3)

### 4.1. finance.payments

Все платежи (включая неуспешные). Источник: Aisystant (каналы 1-5) + бот Aist (6-7) + Directus (8).
...
### 4.2. finance.charge_offs

Только успешные списания. **Основа для биллинг-решений** (М4).
...
CREATE TABLE finance.chat_joins (
    ...
    seminar_code    TEXT,
    invite_source   TEXT NOT NULL DEFAULT 'bot',
...
| М3 | **Биллинг остаётся в Aisystant.** Церен учитывает совместимость схемы | Дима: «Он работает, пока не трогаем» |
...
Биллинг-логика Aisystant:
  ...
  - Решения на основе charge_off (М4)
```
...
|---------|-----|-----------|-----------|
| **Payment Registry** (SYS.011) | Учёт транзакций. `payment` (18 кол.) + `charge_off` (10 кол.). **`charge_off` = основа** (М4) | Внутри монолита Aisystant | Фаза A: dual-write в Neon |
| **Биллинг-логика** (модуль) | Подписки, стажировки, продление. **Остаётся** (М3) | Внутри монолита | Не переносится пока |
