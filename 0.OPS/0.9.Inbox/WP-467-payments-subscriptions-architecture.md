...
**Статус:** решено. Открытый хвост — отдельное маленькое архитектурное решение: как именно расширить `user_axis_state` под множественные product-level права (форма схемы: доп. таблица или JSONB-набор), не сам факт переиспользования паттерна. §4 (целевая архитектура) нужно обновить под направление (г) перед Ф3 ArchGate.
