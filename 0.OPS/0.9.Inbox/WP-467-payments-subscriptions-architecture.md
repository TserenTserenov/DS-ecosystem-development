...
> **Актуализация 2026-09-10 (пир-сессия Claude+Codex, WP-502):** §11 (Д10) уже фиксирует окончательное решение пилота (03.08, направление г) — но §0 п.1 и §14 п.7 ниже всё ещё называли тот же вопрос «открытым»/«критичным блокером» вопреки этому; формулировки исправлены на месте (см. правки ниже).
...
flowchart TB
    ...
    subgraph PROJECTION["Проекция прав (событие → read-model, без JWT/Keto — Д10 направление г)"]
        AXIS["public.user_axis_state: ось belonging — общий тир T0-T4 (одно значение)"]
        ENT["public.user_entitlements: продукт-специфичные права — семинар/когорта/сообщество/пак (много строк на аккаунт)"]
    end

    subgraph CONSUMERS["Сервисы-потребители"]
        SVC["LMS · IWE-сервисы · шлюз · бот · ..."]
    end
    ...
    ORDER --> CONTRACT
    ORDER --> SPLIT
    CONTRACT -- "доменное событие (adapter)" --> AXIS
    CONTRACT -- "доменное событие (adapter)" --> ENT
    AXIS -- "читает напрямую, без токена" --> SVC
    ENT -- "читает напрямую, без токена" --> SVC
```

**Три потока (как в v1, дополнены каталогом и долями; проверка доступа обновлена под Д10 направление г, 03.08 — JWT-флаг для product-level прав снят):**
...
3. **Проверка доступа (без JWT-флага для product-level прав).** `contract` эмитит доменное событие при каждом изменении права → adapter проецирует его в две read-model: `user_axis_state` (общий тир belonging, как решено РП-457, не меняется этой версией) и новую `user_entitlements` (продукт-специфичные права, см. §4.1). Сервис-потребитель читает нужную проекцию напрямую по `account_id`, в биллинг не ходит. Общий флаг «подписчик/не подписчик» (мир, только подписки) по-прежнему можно читать из `user_axis_state.belonging` — Ори-хук в JWT для этого узкого случая не обязателен, но и не запрещён отдельным решением; для product-level прав (Россия, много одновременных грантов) JWT принципиально не используется.

### 4.1 Расширение read-model под product-level права (закрывает открытый хвост Д10, 03.09.2026)

**Решение: отдельная таблица-сестра `public.user_entitlements`, не JSONB внутри `user_axis_state`.** `user_axis_state` хранит ровно одно значение состояния на ось (`PRIMARY KEY (account_id, axis)`) и уже живёт в проде для 3 осей — трогать её форму рискованно без необходимости. JSONB-набор внутри одной строки потребовал бы собственного guard'а на out-of-order запись по каждому ключу набора — тот же `last_event_id`-паттерн, который у отдельной таблицы уже есть на уровне строки, без дублирования логики. Отдельная таблица также даёт простой индексированный запрос «все активные права аккаунта» и «кто имеет право на продукt X» без разбора JSONB.

```sql
CREATE TABLE IF NOT EXISTS public.user_entitlements (
    account_id          UUID        NOT NULL,
    product_code        TEXT        NOT NULL,   -- код из каталога (product/plan, §3)
    status               TEXT        NOT NULL CHECK (status IN ('active', 'expired', 'revoked')),
    granted_at           TIMESTAMPTZ NOT NULL,
    expires_at           TIMESTAMPTZ,            -- NULL = бессрочно (пример: доступ к записи)
    source_contract_id   UUID,                   -- ссылка на contract/order, породивший право — аудит происхождения
    updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    last_event_id        BIGINT      NOT NULL,   -- тот же guard от out-of-order retry, что у user_axis_state
    PRIMARY KEY (account_id, product_code)
);

CREATE INDEX IF NOT EXISTS idx_user_entitlements_product ON public.user_entitlements (product_code) WHERE status = 'active';
```

**Разграничение с `user_axis_state.belonging`:** ось belonging остаётся общим тиром пользователя (T0-T4, тот же смысл, что уже в проде) — не product-level, не меняется этой версией. `user_entitlements` — отдельная, более мелкая проекция: конкретные продукты/паки/когорты/клуб (вход из 17.08 и от Анатолия 25.08), не общий уровень доверия платформе. Сервис-потребитель, которому нужен только общий факт «подписан ли пользователь вообще» (мир, только подписки), читает `belonging`; кому нужно «есть ли у пользователя доступ именно к паку Х» — читает `user_entitlements`.

**Не реализовано этой правкой — только архитектура документа.** Миграция (DDL выше — черновик, не применённый SQL) и код adapter'а — реализация Этапа 0/1 плана (§12), после прохождения Ф3 АрхГейт, не до него.
...
**Бонусы (РП-446):** механизм «Потратить бонусы» в боте практически реализован (Ф1-Ф4b задеплоены): point_balances (баланс), redeemed_events (траты), курс 0,10 ₽/бонус, двухфазный резерв (prepare → reserve → confirm) с откатом просроченных резервов.
...
Бонус — это скидка, не отдельные деньги. Механика полностью переиспользует уже реализованное в РП-446 (точки хранения не меняются: point_balances, redeemed_events, курс 0,10 ₽/бонус, двухфазный резерв).
...
## 8. Как каждый тип продукт ложится на модель

> **Связь с картой ЦА (РП471, Ф5):** эти 11 типов продуктов разложены по трём контурам (человек/организация/сообщество) в `A.Systems-Builder/A1.Teams-and-Communities/A1.1.Meaning/1.1.2. Marketing/Матрица продуктов и контуров 1.1.md`. Сегменты карты (A1/A2/K/C/…) — двери адресации, не продукты; описания аудиторий здесь не дублируются.
...
**Д10 (РЕШЕНО пилотом 03.08 — направление (г). Как продукт-специфичные права (не только «подписчик/не подписчик») попадают в проверку доступа, если П3 — один флаг в JWT?**
...
**Решение пилота (03.08):** направление (г) — не гипотетическая инфраструктура, а уже проверенный, задеплоенный, прошедший ArchGate паттерн этой же платформы для той же проблемы («много состояний вместо одного флага»); не тянет новую зависимость (Keto). Запись решения: `DS-my-strategy/decisions/2026-08/2026-08-03-wp-467-d10-pravo-dostupa-variant-g-user-axis-state.md`.

**Статус:** решено полностью, включая открытый хвост. Форма схемы — отдельная таблица `public.user_entitlements` (не JSONB), см. §4.1. §4 (целевая архитектура) обновлён под направление (г) — JWT-хук для product-level прав снят из диаграммы, проверка доступа идёт через прямое чтение read-model. Готово к Ф3 ArchGate.
...
7. ~~**Пилот (КРИТИЧНО, ревью 10.07, поднято Андреем):** см. Д10 (§11)~~ — **РЕШЕНО пилотом 03.08** (направление г: read-model `user_axis_state` по паттерну WP-457, без Ory Keto). Больше не блокирует §4 и Ф3 ArchGate.
