...
### АП-18: Новый контент сразу в active

**Проблема**: Новый документ создаётся со статусом `active` без ревью.

**❌ Ошибка**:
```yaml
status: active          # ← Новый документ!
created: 2025-01-16
...
created: 2025-01-16
# После ревью → review → active
```

**Правило**: **Новый контент проходит stub → draft → review → active**. Словарь и переходы — см. [05-frontmatter-spec.md](../0.6.Repository-Processes/05-frontmatter-spec.md#status).
...
### АП-23: Папки по статусу зрелости

**Проблема**: Документы разделяются по папкам согласно статусу зрелости (`drafts/`, `proposals/`, `accepted/`) вместо размещения по матрице 3×3 S2R.

**❌ Ошибка**:
```
B.Aisystant-Ecosystem/
├── B3.Ecosystem-Builder/
│   └── B3.1.Meaning/
│       ├── drafts/              # ← Второй словарь размещения!
│       │   └── Regional-Split/
│       ├── proposals/
│       └── accepted/
│           └── ADR-012-regional-split/
```
...
├── B3.Ecosystem-Builder/
│   └── B3.1.Meaning/
│       ├── Regional-Split/          # Сразу в правильном месте по S2R
│       │   └── 00-overview.md       # status: draft в frontmatter
│       └── Привлечение инвестиций.md # status: active в frontmatter
```

**Правило**: **Размещение документа определяется матрицей 3×3 (ядро × система × роль), а статус зрелости — полем `status` в frontmatter.** Это ортогональные измерения — нельзя смешивать их в одном словаре размещения.

**Почему это ошибка**:
1. **Два конфликтующих словаря размещения** — S2R даёт один ответ «куда класть», а статус-папки дают другой. Документ может быть одновременно `draft` и относиться к F3 — непонятно, где он физически.
2. **Ссылки ломаются при промоции** — при `draft → active` приходится делать `git mv`, все `depends_on` / `related` / внешние ссылки (в постах, заметках, commits) ломаются.
3. **История git шумит** — commit переименования перекрывает содержательные изменения, трудно читать blame.
4. **Поиск документов по теме разваливается** — документ ищется по «это про compliance» (S2R), а не по «это уже принято» (статус). При папках по статусу поиск становится двухшаговым.
5. **Нет машиночитаемости** — отчёт «все `draft` с блокерами» требует обхода папок + парсинга имён, а не простого фильтра `status:`.

**Источник**: [05-frontmatter-spec.md#размещение-vs-статус](../0.6.Repository-Processes/05-frontmatter-spec.md#размещение-vs-статус) — каноническое правило.

**Диагностика**: Если видите папку с именем, обозначающим статус жизненного цикла (`drafts/`, `wip/`, `proposals/`, `accepted/`, `archived/` на уровне размещения, а не в `0.99.Archive/`) — это АП-23.

**Решение**: Переместить документы в правильное место по S2R, статус перенести в frontmatter. Обновить все ссылки.
...
|-------------|--------------|----------|
| **Критический** | АП-1, АП-2, АП-3, АП-23 | Исправить немедленно (структура нарушена) |
| **Высокий** | АП-10, АП-11, АП-13 | Исправить в течение недели (метаданные) |
| **Средний** | АП-6, АП-7, АП-12 | Исправить в течение месяца (техдолг) |
| **Низкий** | АП-8, АП-9, АП-20 | Исправить по возможности (качество) |
