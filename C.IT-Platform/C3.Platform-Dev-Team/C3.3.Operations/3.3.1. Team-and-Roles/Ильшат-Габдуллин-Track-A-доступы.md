...
| 2 | **Neon** (16 БД) | ⬜ Нужно выдать | 1. Зайти в [console.neon.tech](https://console.neon.tech) → проект → **Settings** → **Members** → **Invite** → `igabdullin@gmail.com` → роль **Admin** или **Editor** | Event sourcing, bounded contexts |
| 3 | **Railway** | ⬜ Нужно выдать | 1. [dashboard.railway.app](https://dashboard.railway.app) → проект → **Settings** → **Members** → **Invite** → `igabdullin@gmail.com` → роль **Admin** | Python-сервисы, Chatwoot, n8n |
...
| 6 | **Better Stack** | ⬜ Нужно выдать | 1. [betterstack.com](https://betterstack.com) → **Team Settings** → **Invite member** → `igabdullin@gmail.com` → роль **Admin** или **Owner** | Мониторинг, алерты, статус-страницы |
...
| 10 | **Chatwoot** (self-hosted) | ⬜ Нужно выдать | 1. Открыть instance Chatwoot (URL в Railway) → **Settings** → **Agents** → **Add agent** → `igabdullin@gmail.com` → роль **Administrator** | Поддержка, тикеты |
