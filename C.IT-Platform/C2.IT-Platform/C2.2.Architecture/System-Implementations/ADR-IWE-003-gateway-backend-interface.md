...
**Контракт score:** Все backend ДОЛЖНЫ возвращать score в нормализованном диапазоне [0.0, 1.0], где:
- 1.0 = точное совпадение
- 0.0 = нет совпадения
- Текущие реализации: cosine similarity (pgvector) уже в [0, 1]

**Роль Gateway (уточнение 5 апр 2026):** Gateway = прозрачный прокси. Авторизует пользователя, определяет доступные ему MCP, пробрасывает инструменты as-is. Gateway **НЕ выполняет** merge, ранжирование или фильтрацию результатов между бэкендами. Каждый MCP -- отдельный инструмент для LLM-клиента. LLM сама решает, какие инструменты вызывать и как интерпретировать результаты. Если MCP выдаёт много нерелевантных результатов -- проблема в данных MCP (ingestion), а не в Gateway.

**Дедупликация:** при необходимости выполняется на стороне клиента, не Gateway.
...
- Score-нормализация [0,1] обеспечивает единообразие результатов для LLM-клиента
...
**Риски:**
- Backend с нестандартным score-диапазоном может вводить LLM в заблуждение. Митигация: Knowledge Gate KG-11 (калибровка при подключении)
