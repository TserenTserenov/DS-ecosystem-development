...
status: active
created: 2026-05-08
updated: 2026-09-25 (WP-544 А5 — документальная сверка Ф5.1–Ф5.3 и H-301; без нового живого аудита, см. §3.2/§7)
...
### 3.2 Сверка Ф5.1–Ф5.3 (WP-544 А5, 2026-09-25)
...
| Ф5.2 / А1 — квоты и ограничение нагрузки | IP-квота `event-gateway` дала 429 в проде 06.09. Наличие выкатки `payment-receiver` подтверждено сверкой 21.09; прежний вывод «код не задеплоен» отозван. 25.09 локальный стенд workerd прошёл **13/13 проверок**: `payment-receiver` 6/6, `event-gateway` 7/7, включая шесть отрицательных контролей с точечным отключением защиты в тестовой сборке. | **Частично, до отдельного подтверждения пилота.** Стенд проверяет поведение одного изолята, обходя прежнее ограничение теста из-за белого списка IP без ослабления боевой защиты. Действующая конфигурация и срабатывание защиты в проде этим прогоном не подтверждены. Ревью кода Кими и зачёт для H-301 — отдельные шаги. |
...
**Граница нового доказательства А1:** оба набора `tests/wp544-isolated-worker.test.ts` запускают рабочий `src/index.ts` в локальном workerd; исходники сервисов не изменены. Драйвер SQL и ответы внешнего API подставные, остальные исходящие обращения отклоняются. Проверены белый список IP, открытие автовыключателя после пяти ошибок и предел 20 одновременных проверок в `payment-receiver`; IP-квота, квота и предел параллельных запросов источника, а также проверка подписи до расходования его квоты в `event-gateway`. Это семь проверок поведения и шесть отрицательных контролей, а не 13 независимых мер защиты. Наблюдаемые обращения к подставному SQL не подтверждают сохранность записи в настоящей БД, полный путь доставки события или плановый дренаж; А2/А3 этим результатом не закрываются. Глобальный предел 200 запросов шлюза, восстановление автовыключателя после выдержки и распределённое ограничение нагрузки этим набором не проверены.

**H-301:** последнее решение пилота от **21.09 — «недостаточно данных, срок продлён — ждёт теста на изолированном стенде»** ([журнал гипотез, «Сверка H-301»](../../../../DS-my-strategy/current/hypotheses-log.md)). Решение 16.09 «опровергнута» сохраняется как предыдущее; текущий вердикт определяется записью 21.09. Локальный результат А1 от 25.09 добавляет доказательство для рассмотрения, но сам по себе не меняет этот вердикт. Принятые доказательства А2/А4, подтверждение закрытия отдельных строк и результат А1 на стенде — разные условия; сверка документа их не объединяет.
