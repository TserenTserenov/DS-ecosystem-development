...
**Охватывает:**
- Ступени 1–4 программы «Лаборатория развития» (ЛР): Случайный → Практикующий → Систематический → Дисциплинированный
- Переход 4 → 5 (Дисциплинированный → Проактивный) как тройной gate
- Начисление баллов за все действия (все три домена)
- Классификация действий по доменам (learning / practice / work)

**НЕ охватывает:**
- Ступень 5 (Проактивный) — целевое состояние, не переход
...
| **M2** | IWE/ОРЗ — регулярные активные действия | **3→4, 4→5** (не блокирует 1→3) |
...
#### M2 (IWE/ОРЗ) — anchor: % задач через ОРЗ + зрелость артефактов

| idx | % ОРЗ | Proxy из domain_event (за период baseline) |
|-----|-------|---------------------------------------------|
| **1** | 0% | Нет day_plan_opened/closed, нет pack_updated, нет knowledge_extracted |
| **2** | <30% | day_plan_closed эпизодически (< 2/нед), ≤ 1 week_plan_closed за 4 нед |
| **3** | >60% | day_plan_closed ≥ 4/нед, week_plan_closed каждую неделю, pack_updated + knowledge_extracted ≥ 4 за 30 дн |
| **4** | >90% | Всё из idx 3 + month_plan_closed присутствует, pack_updated ≥ 8 за 30 дн, ≥ 5 WP-контекстов активны |
| **5** | 100% | Всё из idx 4 + собственные расширения протоколов (strategy_session_completed регулярно) |
...
| **M2** | baseline_idx ≥ 3 (>60% ОРЗ) | day_plan_closed ≥ 4/нед + week_plan_closed каждую нед + pack_updated + knowledge_extracted ≥ 4 за 30 дн |
...
#### M2.baseline_idx — % задач через ОРЗ

```sql
-- Триада ОРЗ-полноты: day/week/month + Pack-эволюция
WITH metrics AS (
  SELECT
