...
**Нормализация — multi-window:** T проверяется по всем 5 окнам одновременно; берётся наибольший пройденный индекс (WP-310 Ф7, `T_THRESHOLDS` в `stage_config.py`):

| Индекс | Окно | Порог ч/нед |
|:---:|---|:---:|
| **5** | 24 нед | ≥ 10.0 |
| **4** | 12 нед | ≥ 8.0 |
| **3** | 8 нед | ≥ 6.0 |
...
Прямой счёт событий за 30 дней (без весов в MVP коде). Нормализация: 0 → 0 / 1–3 → 1 / 4–10 → 2 / 11–25 → 3 / 26–50 → 4 / > 50 → 5.

| event_type | Включается в счёт |
|-----------|------------------------|
| `lesson_completed` | ✅ |
| `knowledge_extracted` | ✅ |
| `pack_updated` | ✅ |
| `qualification_granted` | ✅ (Gap-В: не реализован в bridge-2) |

> **MVP:** `calc_m()` считает события без весов — `SUM(count per event_type)`. Весовая схема (2/1.5/1/3) была в дизайне, но не реализована. Изменение весов — отдельный РП после wave-1 калибровки.
