...
| S-20 | Процесс промоции скрипта L3→L1: (1) скрипт типа 2 (author) → используется ≥2 пользователями или стабилен 2 недели → (2) выделить universal-часть (убрать авторские пути/константы) → (3) перенести в `FMT/scripts/` → (4) добавить в `update-manifest.json` → (5) удалить из `~/IWE/scripts/` авторского IWE → (6) запустить `update.sh` → (7) убедиться, что скрипт вернулся уже из FMT. Типы и правило размещения: DP.KR.001 §5.6 | `DS-ecosystem-development/.../IWE-staging.md` + DP.KR.001 §5.6 | 2026-04-17 | 1 скрипт прошёл полный цикл L3→L1 без коллизий | testing |
| S-21 | Обязательная проверка коллизий скриптов перед L3→L1 промоцией: `bash` сравнение `~/IWE/scripts/` vs `FMT/scripts/` + `diff`. Если имя уже в FMT — либо мерджить, либо переименовывать перед промоцией. Причина: `backup-icloud.sh`, `check-dirty-repos.sh` уже существуют в обоих местах (обнаружено 17 апр) | `bash`-скрипт или шаг в чеклисте промоции | 2026-04-17 | 2 промоции подряд прошли без коллизии; существующие коллизии разобраны (merge или rename) | testing |
